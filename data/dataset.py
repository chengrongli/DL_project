...
class _BaseSpriteDataset(Dataset):
    ...
    def _apply_augment(
        self, front: Image.Image, back: Image.Image
...
class FrontToBackDataset(_BaseSpriteDataset):
    ...
    def __getitem__(self, idx: int):  # type: ignore[override]
        ...
        if self.augment:
            front_img, back_img = self._apply_augment(front_img, back_img)
            # Apply occlusion only to front (robustness augmentation)
...
def build_and_save_index(sprite_dir: str, out_dir: str,
                          index_path: str) -> List[Tuple[str, str]]:
    """
    ...
    """
    from data.spritesheet_utils import build_pair_dataset
