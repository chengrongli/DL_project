...
from data.spritesheet_utils import spritesheet_to_pair
...
def _save_pair(
    front,  # PIL.Image.Image (kept untyped to avoid optional dependency hints)
    back,
    out_dir: Path,
    repo_root: Path,
    src_path: Path,
...
def extract_pairs_from_repo(
    ...
    *,
    ...
    for sprite_path in candidates:
        ...
        if dry_run:
            front_path = out / sprite_path.name  # placeholder path
