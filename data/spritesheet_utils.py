...
def extract_tile(sheet: Image.Image, row: int, col: int,
...
def extract_front_back(sheet: Image.Image,
                        tile_w: int = TILE_W,
                        tile_h: int = TILE_H,
                        col: int = COL_IDLE) -> Tuple[Image.Image, Image.Image]:
    """
...
def compose_layers(layer_paths: List[str],
...
def collect_spritesheet_pairs(root_dir: str,
                               extensions: Tuple[str, ...] = (".png",)) -> List[str]:
    """
...
def spritesheet_to_pair(path: str,
                         col: int = COL_IDLE) -> Optional[Tuple[Image.Image, Image.Image]]:
    """
    ...
    # Require at least 1 row (down) + 1 row (up) * TILE_H height
...
def save_pair(front: Image.Image, back: Image.Image,
              out_dir: str, stem: str) -> Tuple[str, str]:
...
def build_pair_dataset(sprite_dir: str, out_dir: str,
                        col: int = COL_IDLE) -> List[Tuple[str, str]]:
    """
